...
public class UnitSelectionHandler : MonoBehaviour
{
	...
	private void StartSelectionArea()
	{
		...
		clickStartPos = Mouse.current.position.ReadValue();
	...
	private void UpdateSelectionArea()
	{
		...
		float areaWidth = mousePos.x - clickStartPos.x;
		float areaHeight = mousePos.y - clickStartPos.y;

		unitSelectionArea.sizeDelta = new Vector2(
			Mathf.Abs(areaWidth), Mathf.Abs(areaHeight));
		unitSelectionArea.anchoredPosition = 
			clickStartPos + new Vector2(areaWidth / 2, areaHeight / 2);
	}
